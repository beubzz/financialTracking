...
export async function sendActionEmail(to: string, subject: string, actionUrl: string, actionLabel: string) {
  const text = `${actionLabel}: ${actionUrl}`;
  if (!transporter) {
    console.info(`[mail development] ${to} - ${text}`);
    return;
  }
  await transporter.sendMail({ from: env.MAIL_FROM, to, subject, text, html: `<p>${actionLabel}</p><p><a href="${actionUrl}">${actionUrl}</a></p>` });
}
