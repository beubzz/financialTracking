...
router.post("/register", async (request, response) => {
  ...
  await prisma.verificationToken.create({
    data: {
      userId: user.id,
      tokenHash: tokenHash(rawVerificationToken),
...
router.post("/forgot-password", async (request, response) => {
  ...
  if (parsed.success) {
    ...
    if (user) {
      ...
      await sendActionEmail(
        user.email,
        "Réinitialisez votre mot de passe Ledgerly",
        `${env.FRONTEND_URL}/reset-password?token=${rawResetToken}`,
        "Réinitialisez votre mot de passe Ledgerly",
      );
    }
  }
...
router.post("/reset-password", async (request, response) => {
  const parsed = z
    ...
    .safeParse(request.body);
  if (!parsed.success)
    return response.status(400).json({ error: "Invalid reset request" });
