...
export interface ApiEntry { id: string; label: string; amount: string | number; recurrence: 'WEEK' | 'MONTH' | 'YEAR'; section: 'MANDATORY' | 'PLEASURE' | 'VARIABLE'; type: 'INCOME' | 'EXPENSE'; category?: string | null; note?: string | null; occurredAt?: string | null; }
...
export class FinanceService {
  ...
  addEntry(entry: { label: string; amount: number; section: 'mandatory' | 'pleasure' | 'variable'; recurrence: 'week' | 'month' | 'year'; category?: string; note?: string }) { return this.http.post<{ entry: ApiEntry }>(`${environment.apiUrl}/finance/entries`, entry); }
  updateEntry(id: string, entry: Partial<{ label: string; amount: number; section: 'mandatory' | 'pleasure' | 'variable'; recurrence: 'week' | 'month' | 'year'; category: string; note: string }>, month?: string) { return this.http.patch<{ entry: ApiEntry }>(`${environment.apiUrl}/finance/entries/${id}`, entry, { params: month ? { month } : {} }); }
