...
export class GoalsPage {
  ...
  protected addGoal() {
    ...
    this.finance
      .addGoal({ name: this.name.trim(), target: this.target, saved: this.saved ?? 0 })
      .subscribe({
        next: ({ goal }) => {
  ...
  protected format(value: string | number) {
    return new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(
      Number(value),
    );
  }
  protected deleteGoal(id: string) {
    this.finance
      .deleteGoal(id)
      .subscribe({
        next: () => this.goals.update((goals) => goals.filter((goal) => goal.id !== id)),
      });
  }
  private load() {
    this.finance.getGoals().subscribe(({ goals }) => this.goals.set(goals));
  }
}
