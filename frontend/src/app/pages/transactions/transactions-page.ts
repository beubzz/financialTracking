...
export class TransactionsPage {
  ...
  protected startEdit(entry: ApiEntry) { this.editingId.set(entry.id); this.editingLabel.set(entry.label); this.editingAmount.set(Number(entry.amount)); }
  protected cancelEdit() { this.editingId.set(null); }
  protected saveEdit(entry: ApiEntry) { const label = this.editingLabel().trim(); const amount = this.editingAmount(); if (!label || !amount || amount <= 0) return; this.finance.updateEntry(entry.id, { label, amount }).subscribe({ next: ({ entry: updated }) => { this.entries.update((entries) => entries.map((current) => current.id === updated.id ? updated : current)); this.cancelEdit(); } }); }
}
